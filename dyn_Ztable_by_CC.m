...
%%% i.e., dyn_Ztable_by_CC(table_file_one,table_file_two);
%%% and outputs:
%%%     'table_file_one_bestZ.tbl'
%%%
%%% It will also return the workspace variable if run as:
%%% i.e., bestZ_table] = dyn_Ztable_by_CC(table_file_one,table_file_two)
%%%
%%% Author: TL (UCSD 2021)
function [bestZ_table] = dyn_Ztable_by_CC(table_file_one,table_file_two)

% Check inputs
if nargin ~= 2
    error('dyn_Ztable_by_CC(): takes 2 required input')
end
...
output_table_name = sprintf('%s_bestZ.tbl',table_file_rootname);
...
fprintf('\nOriginal meanCC of 1st table: %d', mean(table_file_one(:,10),1))
fprintf('\nOriginal meanCC of 2nd table: %d', mean(table_file_two(:,10),1))
