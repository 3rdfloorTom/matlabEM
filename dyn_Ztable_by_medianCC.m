...
%%% i.e., dyn_Ztable_by_medianCC(table_file_one,table_file_two);
%%% and outputs:
%%%     'table_file_one_bestZ_byMedianCC.tbl'
%%%
%%% It will also return the workspace variable if run as:
%%% i.e., bestZ_table] = dyn_Ztable_by_medianCC(table_file_one,table_file_two)
%%%
%%% Author: TL (UCSD 2021)
function [bestZ_table] = dyn_Ztable_by_medianCC(table_file_one,table_file_two)

% Check inputs
if nargin ~= 2
    error('dyn_Ztable_by_medianCC(): takes 2 required input')
end
...
if size(table_one,1) ~= size(table_two,1)
    error('table sizes do NOT match!, these are not tables related by just a Z-inversion')
end
...
if size(table_one_region_list,1) ~= size(table_two_region_list,1)
    error('the number of table regions do NOT match!, these are not tables related by just a Z-inversion')
end
...
bestZ_table = [];
    ...
    for i = 1:size(table_one_region_list,1)
        ...
        median_CC_one = median(region_table_one(:,10),1);
        median_CC_two = median(region_table_two(:,10),1);
        
        fprintf('\n 1st table medianCC: %d', median_CC_one)
        fprintf('\n 2nd table medianCC: %d', median_CC_two)        
        
         % Concatenate region_table_# with higher meanCC into bestZ_table
        if median_CC_one > median_CC_two
...
output_table_name = sprintf('%s_bestZ_byMedianCC.tbl',table_file_rootname);
...
fprintf('\nOriginal meanCC of 1st table: %d', median(table_one(:,10),1))
fprintf('\nOriginal meanCC of 2nd table: %d', median(table_two(:,10),1))
fprintf('\nmeanCC of the new table: %d', median(bestZ_table(:,10),1))
